...
describe('Página de pedidos', () => {
  ...
  beforeEach(async () => {
    ...
    const reqPedidos = httpTestingController.expectOne(
      `${environment.urlApi}/api/ventas/cliente/mis-ventas`,
    );
    expect(reqPedidos.request.method).toBe('GET');
    reqPedidos.flush(mockInvoiceResponse);
  ...
  it('Deberían cargar las facturas del cliente y sus productos adquiridos', () => {
    const pedidos = fixture.nativeElement.querySelectorAll(
      '[data-testid="detalle-pedido"]',
    );
    expect(pedidos.length).toBe(mockInvoiceResponse.ventas.length);

    pedidos.forEach((pedido: HTMLElement, index: number) => {
      const totalPedido = pedido.querySelector('[data-testid="total-pedido"]');

      expect(totalPedido?.textContent?.trim()).toBe(
        `$${mockInvoiceResponse.ventas[index].total.toFixed(2)}`,
      );

      const productos = pedido.querySelectorAll(
        '[data-testid="producto-pedido"]',
      ) as NodeListOf<HTMLElement>;

      expect(productos.length).toBe(
        mockInvoiceResponse.ventas[index].productos.length,
      );

      productos.forEach((producto: HTMLElement, productoIndex: number) => {
        const imagenProducto = producto.querySelector(
        ...
        expect(imagenProducto.src).toBe(
          mockInvoiceResponse.ventas[index].productos[productoIndex].imagen,
        );
        expect(cantidadProducto.textContent?.trim()).toBe(
          `Cantidad: ${mockInvoiceResponse.ventas[index].productos[
            productoIndex
          ].cantidad.toString()}`,
        );
        expect(precioProducto.textContent?.trim()).toBe(
          `Precio unit.: $${mockInvoiceResponse.ventas[index].productos[productoIndex].precio.toFixed(2)}`,
        );
      });
    });
  });
